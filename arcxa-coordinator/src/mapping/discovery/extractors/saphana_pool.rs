//! Poolable SAP HANA ODBC Connection Implementation
//!
//! Implements OdbcPoolableConnection trait for SAP HANA databases using
...
use super::saphana::SAPHANAExtractor;
...
/// Poolable SAP HANA ODBC connection
///
...
#[cfg(feature = "odbc")]
pub struct OdbcSAPHANAConnection {
    state: Arc<Mutex<SAPHANAConnectionState>>,
}

#[cfg(feature = "odbc")]
struct SAPHANAConnectionState {
...
#[cfg(feature = "odbc")]
unsafe impl Send for SAPHANAConnectionState {}
#[cfg(feature = "odbc")]
unsafe impl Sync for SAPHANAConnectionState {}

#[cfg(feature = "odbc")]
impl OdbcSAPHANAConnection {
    /// Execute internal query implementation
    fn execute_internal(
        &mut self,
        query: &str,
...
#[cfg(feature = "odbc")]
impl OdbcPoolableConnection for OdbcSAPHANAConnection {
    fn connect(connection_string: &str) -> Result<Self> {
        ...
        tracing::debug!("Connecting to SAP HANA via ODBC");
        ...
        let connection = env_ref
            ...
            .map_err(|e| {
                ...
                anyhow!("Failed to connect to SAP HANA: {:?}", e)
            })?;

        let state = SAPHANAConnectionState {
            _env_ptr: env_ptr,
    ...
    fn is_alive(&self) -> bool {
        // Try to execute simple SAP HANA health check query
        let query = "SELECT 1 FROM DUMMY";
...
#[cfg(feature = "odbc")]
impl Drop for SAPHANAConnectionState {
    fn drop(&mut self) {
        ...
        if let Some(connection) = self.connection.take() {
            ...
            if let Err(panic_info) = drop_result {
                tracing::error!("ODBC library panicked while dropping SAP HANA connection");
        ...
        if !self._env_ptr.is_null() {
            ...
            if let Err(panic_info) = env_drop_result {
                tracing::error!("ODBC library panicked while dropping SAP HANA environment");
                if let Some(s) = panic_info.downcast_ref::<&str>() {
                    tracing::error!("Panic message: {}", s);
                } else if let Some(s) = panic_info.downcast_ref::<String>() {
                    tracing::error!("Panic message: {}", s);
...
#[cfg(feature = "odbc")]
impl Drop for OdbcSAPHANAConnection {
    fn drop(&mut self) {
        ...
        tracing::debug!("ODBC SAP HANA connection dropped");
    }
}
